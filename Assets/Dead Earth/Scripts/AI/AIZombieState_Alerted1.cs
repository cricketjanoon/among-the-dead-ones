...
public class AIZombieState_Alerted1 : AIZombieState
{
    ...
    [SerializeField]
    [Range(1, 60)]
    private float _maxDuration = 10.0f;

    [SerializeField]
    private float _waypointAngleThreshold = 90.0f;

    [SerializeField]
    private float _threatAngleThreshold = 10.0f;

    [SerializeField]
    private float _directionChangeTime = 1.5f;

    // Private Fields
    private float _timer = 0.0f;

    private float _directionChangeTimer = 0.0f;
    private float _screamChance = 0.0f;
    private float _nextScream = 0.0f;

    [SerializeField]
    private float _screamFrequency = 120.0f;

    [SerializeField]
    private float _slerpSpeed = 45.0f;
